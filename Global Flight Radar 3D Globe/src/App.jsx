...
function App() {
  ...
  const [currentScene, setCurrentScene] = useState('intercontinental')
  ...
  const scenes = {
    intercontinental: {
      name: '跨洋洲际长途密集飞行',
      filter: (f) => Math.abs(f.originLat - f.destLat) + Math.abs(f.originLon - f.destLon) > 60
    },
    domestic: {
      name: '国内短途高频穿梭',
      filter: (f) => Math.abs(f.originLat - f.destLat) + Math.abs(f.originLon - f.destLon) <= 60
    ...
    weather: {
      name: '极端天气导致的大面积绕航黄线',
      filter: (f) => f.delayed === 1
    },
    night: {
      name: '深夜低谷期的零星航班',
      filter: (f) => ['CA1234', 'DL5678', 'EK9012'].includes(f.id)
    }
  }

  const filteredFlights = flights.filter(scenes[currentScene].filter)

  return (
    <div className="app">
      <div className="header">
        <h1>Global Flight Radar 3D</h1>
        <SceneSelector
          scenes={scenes}
